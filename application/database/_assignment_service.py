...
def insert_assignment(self:data, conn: Connection,  user_id: int, course_id: int, name: str, deadline: datetime, reveal: datetime, files: list, tasks:List[Task]=[]) -> int:
    ...
    with conn.begin():
        ...
        task_ids=[]
        for task in tasks:
            task_ids.append(self._insert_task(conn, user_id, task.number, id, task.description, task.points, task.files))
        rs.close()
    return id, task_ids
...
def get_assignments_in_time(self:data, conn: Connection, user_id, course_ids, start=utcnow(), end=datetime.datetime.utcnow()+datetime.timedelta(days=7), only_ids=False) -> list:
...
#         sql = sql.select_from(sub.outerjoin(self.submit, (sub.c.sub_task_id == self.submit.c.task_id) & (self.submit.c.owner_id == user_id))).group_by(sub.c.assig_id).order_by(text("1"))
    ...
    if only_ids:
        ...
        sub = select([self.assignment.c.id.label("assig_id"), self.course.c.abbreviation.label("course_abbreviation"), self.task.c.id.label("sub_task_id"), 
        self.assignment.c.name.label("assig_name"), self.assignment.c.reveal.label("assig_reveal"), self.assignment.c.deadline.label("assig_deadline"), self.assignment.c.course_id.label("assig_course_id")])
        ...
        sql = select([sub.c.assig_deadline, agg_sub.c.agg_assig_id, sub.c.assig_course_id, agg_sub.c.task_count, agg_sub.c.submit_count, sub.c.assig_name, sub.c.assig_reveal, sub.c.course_abbreviation ])
        j3 = agg_sub.join(sub,agg_sub.c.agg_assig_id==sub.c.assig_id)
        sql = sql.select_from(j3).order_by(text("1"))
    with conn.begin():
        rs = conn.execute(sql)
        if only_ids:
            return rs.fetchall()
        ...
        for row in rs:
            print(row)
            res.append(Assignment(row[agg_sub.c.agg_assig_id], row[sub.c.assig_name], row[sub.c.assig_reveal], row[sub.c.assig_deadline], [], files=[], task_count= row["task_count"], submit_count=row["submit_count"], course_id = row[sub.c.assig_course_id], course_abbreviation=row[sub.c.course_abbreviation]))
