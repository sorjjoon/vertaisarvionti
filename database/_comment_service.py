...
def insert_comment_dict(self:data, conn: Connection,  comment_dict, files=[]):
    self.logger.info("inserting comment from dictionary, values: %s", comment_dict)
    sql = self.comment.insert().values(comment_dict)
    with conn.begin():
        rs = conn.execute(sql)
        id = rs.inserted_primary_key[0]
        if files:
            self.insert_files(conn, comment_dict["user_id"], files, comment_id=id)
...
def delete_comment(self:data, conn: Connection,  comment_id:int, user_id:int):
    """Delete given comment id. checks rights by making sure user owns the comment before deleting
    ...
    Args:
        ...
        user_id (int): [description]
    """
    sql = self.comment.delete().where((self.comment.c.id==comment_id) & (self.comment.c.user_id==user_id))
    ...
    with conn.begin():
        ...
        for row in rs:
            ...
            if not seen_comments.get(comment_id):
                c = Comment(comment_id, row[self.comment.c.user_id], row[self.comment.c.text], reveal=row[self.comment.c.reveal], date=row[self.comment.c.timestamp], modified=row[self.comment.c.modified], owner_str=owner_str, files=[])
